...
async function init(){const snap=await getDocs(collection(db,"albergue_animales"));state.all=snap.docs.map(d=>d.data());fillFilters();apply();}
...
init().catch(()=>{els.cardsGrid.innerHTML='<div class="empty">No se pudo cargar el albergue. Revisá la configuración Firebase.</div>'});
